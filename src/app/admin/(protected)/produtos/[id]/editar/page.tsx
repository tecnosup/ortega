...
export default function EditarProdutoPage() {
  ...
  const [state, formAction, pending] = useActionState(updateProdutoAction, null);
  ...
  useEffect(() => {
    fetch(`/api/admin/produtos/${id}`)
      ...
      .then((data) => {
        ...
        setImageUrl(data.produto?.imagem ?? "");
        setLoading(false);
      });
  }, [id]);
  ...
  if (!produto) return <p className="text-gray-500 text-sm">Produto não encontrado.</p>;
    ...
    <div className="max-w-xl mx-auto flex flex-col gap-6">
      ...
      <form action={formAction} className="flex flex-col gap-5">
        ...
        <div className="flex flex-col gap-1">
          ...
          <input name="titulo" required defaultValue={produto.titulo} className={inp} />
        </div>
        <div className="flex flex-col gap-1">
          <label className="text-sm font-medium text-gray-400">Descrição</label>
          <textarea name="descricao" rows={3} defaultValue={produto.descricao} className={`${inp} resize-none`} />
        </div>
        <div className="flex flex-col gap-1">
        ...
        <div className="flex flex-col gap-1">
          ...
          <input name="preco" defaultValue={produto.preco} className={inp} />
        </div>
        <div className="flex flex-col gap-1">
          <label className="text-sm font-medium text-gray-400">Status</label>
          <select name="status" defaultValue={produto.status} className={inp}>
        ...
        <div className="flex flex-col gap-1">
          ...
          <input name="order" type="number" defaultValue={produto.order} className={inp} />
        </div>
